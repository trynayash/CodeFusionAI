...
const javascriptTemplate = `// JavaScript Online Compiler - CodeFusion AI
console.log("Hello, JavaScript World!");

// Variables and Data Types
const name = "CodeFusion AI";
const version = 2024;
...
export default function JavaScriptEditor() {
  const [code, setCode] = useState(javascriptTemplate);
  const [output, setOutput] = useState('');
  const [title, setTitle] = useState('JavaScript Project');
    ...
    if (currentSnippetId) {
      ...
      if (error) {
        ...
        toast({
          ...
          description: "Your JavaScript code has been updated successfully.",
        });
        loadSnippets();
      }
    } else {
      ...
      if (error) {
        ...
        toast({
          title: "Snippet saved",
          description: "Your JavaScript code has been saved successfully.",
        });
        loadSnippets();
      }
    }
  ...
  const runCode = useCallback(async () => {
    if (!code.trim()) {
      toast({
        ...
        description: "Please write some JavaScript code first.",
        variant: "destructive",
      });
      return;
    }
    ...
    terminal.addLine(`⚡ Starting JavaScript execution...`, 'info');
    ...
    try {
      ...
      if (result.success) {
        terminal.addLine(`✅ JavaScript execution completed successfully`, 'success');
        ...
        toast({
          title: "JavaScript code executed successfully!",
        ...
        terminal.addLine(`❌ JavaScript execution failed`, 'error');
      ...
      setOutput(`Runtime Error:\n${errorMsg}\n\nThis error occurred during JavaScript execution.`);
      setActiveTab('terminal');
      
      toast({
        title: "Runtime Error",
        description: `JavaScript execution failed: ${errorMsg}`,
        variant: "destructive",
      });
    } finally {
      setIsRunning(false);
      terminal.setRunning(false);
      terminal.addLine(`🏁 JavaScript execution finished`, 'info');
    }
  ...
  const deleteSnippet = async (snippetId: string) => {
    ...
    if (error) {
      ...
      toast({
        ...
        description: "JavaScript code snippet has been deleted successfully.",
      });
      loadSnippets();
            ...
            <div className="flex items-center space-x-4 flex-wrap gap-2 min-w-0 flex-1">
              <div className="flex items-center space-x-3">
                <div className="p-2 bg-gradient-to-r from-yellow-500 to-yellow-600 rounded-lg shadow-lg">
                  <img src={javascriptIcon} alt="JavaScript" className="w-5 h-5" />
                </div>
                <div className="hidden sm:block">
                  <h1 className="font-bold text-lg bg-gradient-to-r from-yellow-600 to-yellow-700 bg-clip-text text-transparent">
                    JavaScript Editor
                  </h1>
                  <p className="text-xs text-slate-500 dark:text-slate-400">Professional JavaScript IDE</p>
                </div>
              </div>
              ...
              <div className="flex-1 max-w-md">
                <Input
                  ...
                  className="bg-white/50 dark:bg-slate-800/50 border-slate-200 dark:border-slate-700 focus:border-yellow-500 dark:focus:border-yellow-400 transition-all duration-200"
                  placeholder="Enter JavaScript project name..."
                />
              </div>
            </div>
            ...
            >
              <div className="p-6">
                <div className="flex items-center justify-between mb-6">
                  <div>
                    <h3 className="text-lg font-bold text-slate-800 dark:text-slate-200">JavaScript History</h3>
                    <p className="text-sm text-slate-500 dark:text-slate-400">Your saved JavaScript projects</p>
                  </div>
                  <Button
                    variant="ghost"
                    size="sm"
                  ...
                  <div className="space-y-3">
                    {snippets.length === 0 ? (
                      <div className="text-center py-8">
                        ...
                        <p className="text-sm text-slate-500 dark:text-slate-400">No saved JavaScript projects yet</p>
                              ...
                              <div className="flex-1 min-w-0" onClick={() => loadSnippet(snippet)}>
                                <div className="flex items-center space-x-3 mb-2">
                                  <div className="w-8 h-8 bg-gradient-to-r from-yellow-500 to-yellow-600 rounded-lg flex items-center justify-center flex-shrink-0">
                                    <img src={javascriptIcon} alt="JavaScript" className="w-4 h-4" />
                                  </div>
                                  <div className="min-w-0 flex-1">
                                    <h4 className="font-semibold text-slate-800 dark:text-slate-200 truncate">{snippet.title}</h4>
                                    <p className="text-sm text-slate-500 dark:text-slate-400">JavaScript</p>
                                  </div>
                                </div>
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-4 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border-b border-slate-200/50 dark:border-slate-700/50">
              <div className="flex items-center space-x-4 w-full sm:w-auto mb-3 sm:mb-0">
                <div className="flex items-center space-x-2 bg-yellow-50 dark:bg-yellow-900/20 px-3 py-2 rounded-lg">
                  <img src={javascriptIcon} alt="JavaScript" className="w-5 h-5" />
                  <span className="font-medium text-yellow-700 dark:text-yellow-300">JavaScript ES2023</span>
                </div>
              </div>
              ...
              <Button 
                onClick={runCode} 
                disabled={isRunning} 
                className="bg-gradient-to-r from-yellow-500 to-yellow-600 hover:from-yellow-600 hover:to-yellow-700 text-white shadow-lg hover:shadow-xl transition-all duration-200 w-full sm:w-auto"
              >
                {isRunning ? (
                  <>
                  ...
                  <div className="space-y-4">
                    {output ? (
                      <motion.div
                        ...
                        <div className={`flex items-center gap-3 mb-4 pb-3 border-b ${
                          ...
                          <span className="font-bold text-sm">
                            {output.includes('Error') ? 'JavaScript Execution Failed' : 'JavaScript Execution Successful'}
                          </span>
                          {executionResult && (
                      ...
                      <div className="text-center py-12">
                        ...
                        <h3 className="text-lg font-semibold text-slate-800 dark:text-slate-200 mb-2">Ready to Execute JavaScript</h3>
                        <p className="text-sm text-slate-500 dark:text-slate-400 mb-6">Click "Run JavaScript Code" to see your output here</p>
                        <div className="bg-slate-50 dark:bg-slate-800/50 rounded-lg p-4 max-w-sm mx-auto">
                          <p className="text-xs font-semibold text-slate-600 dark:text-slate-300 mb-2">⚡ JavaScript Tips:</p>
                          <div className="text-xs text-slate-500 dark:text-slate-400 space-y-1 text-left">
                            <p>• Use console.log() to display output</p>
                            <p>• Declare variables with const/let</p>
                            <p>• Use modern ES6+ features</p>
                            <p>• Handle async operations properly</p>
                          </div>
                        </div>
                      </div>
                    )}
