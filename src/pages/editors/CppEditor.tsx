...
const cppTemplate = `// C++ Online Compiler - CodeFusion AI
#include <iostream>
#include <vector>
#include <string>
#include <algorithm>
...
// Function demonstrating STL usage
void demonstrateSTL() {
    ...
    // Map demonstration
    map<string, int> languagePopularity = {
        {"C++", 95},
        {"Python", 98},
        {"JavaScript", 92},
        {"Java", 90}
    ...
    cout << "\\nLanguage Popularity Scores:" << endl;
    for (const auto& pair : languagePopularity) {
...
void demonstrateSmartPointers() {
    ...
    // shared_ptr
    shared_ptr<Student> student2 = make_shared<Student>("Bob", 22);
    student2->addCourse("Algorithms");
    student2->addCourse("Software Engineering");
...
int main() {
    ...
    cout << "Welcome to CodeFusion AI - C++ Compiler" << endl;
    
    // Basic variables and operations
    string appName = "CodeFusion AI";
    int version = 2024;
    bool isAwesome = true;
    
    cout << "\\nApp: " << appName << " - Version: " << version << endl;
    cout << "Is it awesome? " << (isAwesome ? "Yes" : "No") << endl;
    
    // Array and loops
    vector<string> languages = {"C++", "Python", "JavaScript", "Java", "Go"};
    cout << "\\nSupported Languages:" << endl;
    ...
    // Auto type deduction
    auto message = "C++ is powerful and efficient!";
    cout << "\\nMessage: " << message << endl;
    
    cout << "\\nC++ program executed successfully!" << endl;
...
export default function CppEditor() {
  ...
  const [title, setTitle] = useState('C++ Project');
    ...
    if (currentSnippetId) {
      ...
      if (error) {
        ...
        toast({
          ...
          description: "Your C++ code has been updated successfully.",
        });
        loadSnippets();
      }
    } else {
      ...
      if (error) {
        ...
        toast({
          title: "Snippet saved",
          description: "Your C++ code has been saved successfully.",
        });
        loadSnippets();
      }
    }
  ...
  const runCode = useCallback(async () => {
    ...
    terminal.addLine(`⚡ Starting C++ compilation and execution...`, 'info');
    terminal.addLine(`📝 Code length: ${code.length} characters`, 'info');
    
    try {
      const result = await executionService.executeCode(code, 'cpp', {
        timeout: 15000, // C++ needs time for compilation
        maxMemory: 512, // C++ may need more memory
      ...
      if (result.success) {
        terminal.addLine(`✅ C++ compilation and execution completed successfully`, 'success');
        ...
        terminal.addLine(`❌ C++ compilation/execution failed`, 'error');
      ...
      setOutput(`Runtime Error:\n${errorMsg}\n\nThis error occurred during C++ execution.`);
      setActiveTab('terminal');
      
      toast({
        title: "Runtime Error",
        description: `C++ execution failed: ${errorMsg}`,
        variant: "destructive",
      });
    } finally {
      setIsRunning(false);
      terminal.setRunning(false);
      terminal.addLine(`🏁 C++ execution finished`, 'info');
    }
  ...
  const deleteSnippet = async (snippetId: string) => {
    ...
    if (error) {
      ...
      toast({
        ...
        description: "C++ code snippet has been deleted successfully.",
      });
      loadSnippets();
            ...
            <div className="flex items-center space-x-4 flex-wrap gap-2 min-w-0 flex-1">
              <div className="flex items-center space-x-3">
                <div className="p-2 bg-gradient-to-r from-purple-500 to-purple-600 rounded-lg shadow-lg">
                  <img src={cppIcon} alt="C++" className="w-5 h-5" />
                </div>
                <div className="hidden sm:block">
                  <h1 className="font-bold text-lg bg-gradient-to-r from-purple-600 to-purple-700 bg-clip-text text-transparent">
                    C++ Editor
                  </h1>
                  <p className="text-xs text-slate-500 dark:text-slate-400">Professional C++ IDE</p>
                </div>
              </div>
              ...
              <div className="flex-1 max-w-md">
                <Input
                  ...
                  className="bg-white/50 dark:bg-slate-800/50 border-slate-200 dark:border-slate-700 focus:border-purple-500 dark:focus:border-purple-400 transition-all duration-200"
                  placeholder="Enter C++ project name..."
                />
              </div>
            </div>
            ...
            >
              <div className="p-6">
                <div className="flex items-center justify-between mb-6">
                  <div>
                    <h3 className="text-lg font-bold text-slate-800 dark:text-slate-200">C++ History</h3>
                    <p className="text-sm text-slate-500 dark:text-slate-400">Your saved C++ projects</p>
                  </div>
                  <Button
                    variant="ghost"
                    size="sm"
                  ...
                  <div className="space-y-3">
                    {snippets.length === 0 ? (
                      <div className="text-center py-8">
                        ...
                        <p className="text-sm text-slate-500 dark:text-slate-400">No saved C++ projects yet</p>
                              ...
                              <div className="flex-1 min-w-0" onClick={() => loadSnippet(snippet)}>
                                <div className="flex items-center space-x-3 mb-2">
                                  <div className="w-8 h-8 bg-gradient-to-r from-purple-500 to-purple-600 rounded-lg flex items-center justify-center flex-shrink-0">
                                    <img src={cppIcon} alt="C++" className="w-4 h-4" />
                                  </div>
                                  <div className="min-w-0 flex-1">
                                    <h4 className="font-semibold text-slate-800 dark:text-slate-200 truncate">{snippet.title}</h4>
                                    <p className="text-sm text-slate-500 dark:text-slate-400">C++</p>
                                  </div>
                                </div>
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-4 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border-b border-slate-200/50 dark:border-slate-700/50">
              <div className="flex items-center space-x-4 w-full sm:w-auto mb-3 sm:mb-0">
                <div className="flex items-center space-x-2 bg-purple-50 dark:bg-purple-900/20 px-3 py-2 rounded-lg">
                  <img src={cppIcon} alt="C++" className="w-5 h-5" />
                  <span className="font-medium text-purple-700 dark:text-purple-300">C++20</span>
                </div>
              </div>
              ...
              <Button 
                onClick={runCode} 
                disabled={isRunning} 
                className="bg-gradient-to-r from-purple-500 to-purple-600 hover:from-purple-600 hover:to-purple-700 text-white shadow-lg hover:shadow-xl transition-all duration-200 w-full sm:w-auto"
              >
                {isRunning ? (
                  <>
                  ...
                  <div className="space-y-4">
                    {output ? (
                      <motion.div
                        ...
                        <div className={`flex items-center gap-3 mb-4 pb-3 border-b ${
                          ...
                          <span className="font-bold text-sm">
                            {output.includes('Error') ? 'C++ Compilation/Execution Failed' : 'C++ Compilation/Execution Successful'}
                          </span>
                          {executionResult && (
                      ...
                      <div className="text-center py-12">
                        ...
                        <h3 className="text-lg font-semibold text-slate-800 dark:text-slate-200 mb-2">Ready to Compile C++</h3>
                        <p className="text-sm text-slate-500 dark:text-slate-400 mb-6">Click "Compile & Run C++" to see your output here</p>
                        <div className="bg-slate-50 dark:bg-slate-800/50 rounded-lg p-4 max-w-sm mx-auto">
                          <p className="text-xs font-semibold text-slate-600 dark:text-slate-300 mb-2">⚡ C++ Tips:</p>
                          <div className="text-xs text-slate-500 dark:text-slate-400 space-y-1 text-left">
                            <p>• Include necessary headers</p>
                            <p>• Use std namespace or std:: prefix</p>
                            <p>• Remember to return 0 from main()</p>
                            <p>• Use modern C++ features (C++11+)</p>
                          </div>
                        </div>
                      </div>
                    )}
